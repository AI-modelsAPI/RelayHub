...
- **Authenticity Probes**: Every 12h (`verify_probe_interval`) each routable channel gets a tiny canary through its own credential, egress and identity headers: the model must repeat random words, tool-capable models must honour a forced tool call, and the response `model` field and prompt-token fingerprint are compared. Channels caught serving another model are demoted in routing (still used when nothing else can serve). See [docs/operations.md](docs/operations.md) §5.
- **Stream Failover & Breaker Recovery**: A 2xx stream is held back (`stream_commit_window`, default 10s) until its first output event, so an upstream that answers 200 and then sends an error, an HTML page or nothing at all fails over transparently; with no other channel the client gets the upstream's real status instead of a 200 wrapping an error. Tripped channels come back through one `max_tokens=1` probe (`health_probe_interval`, default 30s) instead of being handed real traffic, and an error the upstream reports inside a committed stream still reaches the client and the usage/trust records. See [docs/operations.md](docs/operations.md) §6.
...
- **Notifications**: Webhook / Bark / Telegram for check-in failures, manual fallbacks, low or exhausted quota, breaker trips and recoveries, and channels failing an authenticity probe, with per-channel cooldown.
