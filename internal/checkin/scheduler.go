	...
	for attempt := 0; attempt <= s.cfg.MaxRetries; attempt++ {
		...
		if errors.Is(lastErr, adapter.ErrNeedWebview) {
			...
			if executor != nil && bInfo.Available {
				...
				// Browser check-ins exit through the same egress as the HTTP
				// adapter paths (AUDIT RH-10): the wired resolver applies the
				// global default; without one the channel proxy still holds.
